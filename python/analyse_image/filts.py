...
def ddist(t):
    ...
    return switchQuad(res[:,:t])

def ellipse(p, q):
    return (vdist(p, q) * (q // 2)) ** 2 + (vdist(q, p).T * (p // 2)) ** 2
...
def circleCut(p, q, p0):
    return 20000 * ellipse(p, q) <= ((p // 2) * (q // 2) * p0) ** 2
...
def vgaussd(t, p0):
    return - vdist(t,t) * np.exp(-vdist(t, t) ** 2 * 40000 / (t * p0)  ** 2)

def dgaussd(t, p0):
    return - ddist(t) * np.exp(-ddist(t) ** 2 * 40000 / (t * p0) ** 2)

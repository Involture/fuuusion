...
class edge:
    ...
    def faceIntersection(self, face):
        ...
        if normal.dotProduct(vp - v0) * normal.dotProduct(vn - v0) > 0:
            ...
            interVect = vn + ((vp - vn) * normal.dotProduct(v0 - vn) /
                              normal.dotProduct(vp - vn))
...
class polyhedron:
    ...
    def addVertex(self, x, y, z):
        ...
        Creates a vertex at given coordinates and adds it to <self>,
            and returns it."""
        newV = vertex(x, y, z)
...
class polyIntersection:
    ...
    def getIntersector(self, v):
        ...
        return next((i for i in self.inter1 if i.v == v),
                    next((i for i in self.inter2 if i.v == v),
                         False))
    ...
    def getIntersectorList(self, l):
        ...
        return ([i for i in self.inter1 if i.v in l] +
                [i for i in self.inter2 if i.v in l])
    ...
    def nextIntersectors(self, inter):
        ...
        if otherI1 is False:
            ...
            print(otherFInters)
            otherI1 = next(filter(lambda x: x.f == inter.pe.pFace,
                                  otherFInters),
                           False)
            if otherI1 is False:
        ...
        if otherI2 is False:
            ...
            otherI2 = next(filter(lambda x: x.f == inter.pe.nFace,
                                  otherFInters),
                           False)
            if otherI2 is False:
                raise ValueError('No intersector found')
    ...
    def traceSurface(self, inter):
        ...
        while nextI != inter:
            ...
            self.result.edges.append(edge(self.newVertex(i),
                                          self.newVertex(nextI)))
            i.sTraced = True
            i = nextI
    ...
    def traceInterior(self, inter):
        ...
        if nI is not False:
            self.result.edges.append(edge(self.newVertex(inter),
                                          self.newVertex(nI)))
            nI.iTraced = True
        else:
            newVert = vert.copy()
        ...
        for j in range(len(self.poly1.faces)):
            ...
            while i < len(f.vertices):
                if (not f.vertices[i - 1].interior) and f.vertices[i].interior:
                    ...
                    while f.vertices[i % n].interior:
                        nV = self.newVertex(f.vertices[i % n])
                        newFace.vertices.append(nV)
                        i += 1
                    lastInter = self.getIntersector(f.vertices[i % n])
                    normal = f.normalVect()
                    vect = (vector(f.vertices[i % n]) -
                            vector(f.vertices[(i - 1) % n]))

                    def f(x):
                        return 0 < normal.dotProduct(vect *
                                                     (vector(x.v) -
                                                      vector(lastInter.v)))
                    inter = next(filter(f, lastInter.adjacents))
            ...
            if not found:
                if f.vertices[0].interior:
                    self.result.faces.append(face([self.newVertex(v)
                                                   for v in f.vertices]))
